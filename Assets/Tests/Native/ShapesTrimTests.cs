...
public class ShapesTrimTests
{
    ...
    public void IsTrimmed_EmptyShape_ReturnsTrue()
    {
        ...
        Assert.IsTrue(ReadOnlyGridShapeExtension.IsTrimmed(ref shape, freeValue: false));
        shape.Dispose();

        var shape2 = new GridShape(5, 0, Allocator.Temp);
        Assert.IsTrue(ReadOnlyGridShapeExtension.IsTrimmed(ref shape2, freeValue: false));
        shape2.Dispose();

        var shape3 = new GridShape(0, 5, Allocator.Temp);
        Assert.IsTrue(ReadOnlyGridShapeExtension.IsTrimmed(ref shape3, freeValue: false));
        shape3.Dispose();
    }
    ...
    public void IsTrimmed_ShapeWithEmptyTopRow_ReturnsFalse()
    {
        ...
        Assert.IsFalse(ReadOnlyGridShapeExtension.IsTrimmed(ref shape, freeValue: false));
        shape.Dispose();
    }
    ...
    public void IsTrimmed_CrossShape_ReturnsTrue()
    {
        ...
        shape[1, 0] = true; // Top
        shape[0, 1] = true; // Left
        shape[1, 1] = true; // Center
        shape[2, 1] = true; // Right
        shape[1, 2] = true; // Bottom
    ...
    public void IsTrimmed_ReadOnlyShape_WorksCorrectly()
    {
        ...
        Assert.IsTrue(ReadOnlyGridShapeExtension.IsTrimmed(in readOnly, freeValue: false));
    ...
    public void Trim_AlreadyTrimmedShape_ReturnsIdenticalCopy()
    {
        ...
        for (var y = 0; y < 3; y++)
        for (var x = 0; x < 3; x++)
            Assert.AreEqual(shape[x, y], trimmed[x, y]);
    ...
    public void Trim_SparseShape_PreservesRelativePositions()
    {
        ...
        shape[2, 1] = true; // Top-left of trimmed area
        shape[4, 1] = true; // Top-right of trimmed area
        shape[3, 2] = true; // Middle
        shape[2, 3] = true; // Bottom-left of trimmed area
        shape[4, 3] = true; // Bottom-right of trimmed area
        ...
        Assert.IsTrue(trimmed[0, 0]);  // Was (2,1)
        Assert.IsTrue(trimmed[2, 0]);  // Was (4,1)
        Assert.IsTrue(trimmed[1, 1]);  // Was (3,2)
        Assert.IsTrue(trimmed[0, 2]);  // Was (2,3)
        Assert.IsTrue(trimmed[2, 2]);  // Was (4,3)

        // Verify empty cells
        Assert.IsFalse(trimmed[1, 0]);
        Assert.IsFalse(trimmed[0, 1]);
        Assert.IsFalse(trimmed[2, 1]);
        Assert.IsFalse(trimmed[1, 2]);
    ...
    public void Trim_LShape_MaintainsShape()
    {
        ...
        Assert.IsTrue(trimmed[0, 0]);
        Assert.IsTrue(trimmed[0, 1]);
        Assert.IsTrue(trimmed[0, 2]);
        Assert.IsTrue(trimmed[1, 2]);
        Assert.IsTrue(trimmed[2, 2]);

        // Verify empty cells
        Assert.IsFalse(trimmed[1, 0]);
        Assert.IsFalse(trimmed[2, 0]);
        Assert.IsFalse(trimmed[1, 1]);
        Assert.IsFalse(trimmed[2, 1]);
